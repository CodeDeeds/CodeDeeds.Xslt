# Compatebility of XSLT versions

## Xslt 1.0 Compatibility
...
`include-content-type`, both HTML-method refinements, are not implemented.

Not implemented: the list types (`xs:NMTOKENS` and its relatives), and the schema-aware forms of everything.
...
Still missing here: the serialization attributes of `xsl:result-document` are attribute value templates, and
are read as literals. A stylesheet computing its `method` or its `format` gets the text of the template
...
Not implemented: `fn:function-lookup`, which needs a name resolved against the static context at run time, and
partial application (`concat('a', ?)`).
...
**Not implemented:** a static variable declared in an *imported* module is not visible to a `use-when` in the
module importing it. Imports are followed after the importing module's declarations have been gathered, and
gathering is what answers its `use-when` expressions; putting imports first would reorder more than this
feature is worth. Within one module, includes spliced and all, the ordering is right.
...
**Not implemented:** the signature check on an override (`XTSE3070`), choosing among several versions of a used
package by `package-version`, `xsl:original` for a variable or an attribute set, and re-exposing through
`xsl:expose` a component a package accepted rather than declared.
...
**Not implemented:** `xsl:sort` over a selection that is not all nodes. The suite does not exercise the
combination, so implementing it would be a claim rather than a feature.
...
the same limit the QT3 run reports; 2506 asks for German number words, and English is the one language this
engine has (see *What is not implemented, and will not be*); and 1004 on the 2.0 run wants a 2.0 processor
...
**`XsltVersion.Implemented` still says `V20`**, so a caller who names no version gets a 2.0 processor: it
answers `2` to `system-property('xsl:version')` and reads a `version="3.0"` stylesheet forwards-compatibly,
where an `xsl:mode` at the top level is *ignored* and `expand-text="yes"` is ignored. A caller who asks for
3.0 gets a processor that reads 3.0's vocabulary and now says so when asked (see *What a processor asked to
be 3.0 says it is*), which is the claim made where it is true rather than everywhere. Moving the default is
the last thing and not the first: every 3.0 element and attribute is in `XsltElements`, which was the
condition set for it, and what remains is the list above — a package model still missing the two cases it
names is not yet what a caller who asked for nothing should be handed.

### Measuring the 3.0 work

The QT3 driver runs the XPath 2.0 subset by default, that being the version the engine claims. `--31` takes in
