...
| | |
| --- | --- |
| XSLT | 3.0, claimed by default. `XsltOptions.Version = XsltVersion.V20` gives a 2.0 processor: `system-property('xsl:version')` answers `2.0`, the vocabulary is 2.0's, and a `version="3.0"` stylesheet is read forwards-compatibly. A `version="1.0"` stylesheet runs with 1.0's semantics on either. |
...
| Number words (`xsl:number`, `format-integer`) | Cardinal and ordinal words in English and German. Numbering by any Unicode decimal-digit family, Latin letters and Roman numerals. | Other languages fall back to English. Other numbering sequences, such as circled digits or Greek letters, fall back to decimal digits. |
| Date and time names (`format-date` and relatives) | English month, day and era names; the Gregorian calendar as `AD` or `ISO`. | Other languages get English with the `[Language: en]` prefix the specification prescribes; other calendars get `[Calendar: AD]`. |
